...
import pl.mamooth.eclipse.magento.handlers.InsertModelHandler;
...
public class InsertModelDialog extends Dialog {
	...
	private InsertModelHandler action;
	...
	class UpdateGroupCombo extends UIJob {
		...
		public IStatus runInUIThread(IProgressMonitor monitor) {
			try {
				modelGroups = module.getModelGroups();
	...
	class UpdateModelCombo extends UIJob {
		...
		public IStatus runInUIThread(IProgressMonitor monitor) {
			try {
				String classBase = module.getClassBaseForModelGroup(group);
	 ...
	 */
	public InsertModelDialog(Shell parentShell, InsertModelHandler insertModelHandler, MagentoModule module) throws XPathExpressionException, ParserConfigurationException, CoreException {
		super(parentShell);
		this.action = insertModelHandler;
		this.module = module;
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		lblModel.setText(I18n.get("model"));
	...
	protected void dialogChanged() {
		...
		if (comboModel.getText().length() == 0) {
			setError(I18n.get("empty_model"));
			return;
		}
		...
		if (!isSelected(comboModel)) {
			setWarining(I18n.get("custom_model"));
		}
		if (!isSelected(comboGroup)) {
	...
	protected boolean isSelected(Combo combo) {
		int index = Arrays.binarySearch(combo.getItems(), combo.getText());
		return index >= 0;
	}

	protected void updateComboGroup() {
		try {
			baseClass = module.getClassBaseForModelGroup(comboGroup.getText());
			if (baseClass.length() == 0) {
				String base = comboGroup.getText();
				if (base.indexOf('_') < 0) {
					base = "mage_" + base;
				}
				baseClass = StringHelper.toCamelCase(base, true, true) + '_' + FolderHelper.MODEL_FOLDER;
			}
			updateClass();
			if (isSelected(comboGroup)) {
	...
	protected void updateClass() {
		if (baseClass != null && baseClass.length() > 0 && comboModel.getText().length() > 0)
			textClass.setText(StringHelper.toCamelCase(baseClass + '_' + comboModel.getText(), true, true));
	}
}
